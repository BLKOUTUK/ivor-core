...
const journeyAwareService = new JourneyAwareConversationService_js_1.JourneyAwareConversationService(conversationService);
// Middleware
...
app.post('/api/chat', async (req, res) => {
    try {
        ...
        const journeyResponse = await journeyAwareService.generateJourneyAwareResponse(message, userContext, sessionId);
        res.json({
            ...journeyResponse,
            sessionId,
...
app.post('/api/emergency', async (req, res) => {
    try {
        ...
        const emergencyResponse = await journeyAwareService.getEmergencyResponse(message, userContext);
        res.json({
            ...emergencyResponse,
            timestamp: new Date().toISOString(),
            domain: 'core',
...
app.get('/api/crisis/resources', async (req, res) => {
    try {
        ...
        const crisisResponse = await journeyAwareService.getEmergencyResponse(`I need crisis resources for ${categories || 'emergency support'}`, { location: location || 'unknown' });
        res.json({
            success: true,
...
function generateWellnessInsights(answers) {
    return [
...
function generateWellnessRecommendations(answers) {
    return [
...
function getNextWellnessSection(currentSectionId) {
...
exports.default = app;
//# sourceMappingURL=server-journey-aware.js.map