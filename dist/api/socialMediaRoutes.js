...
router.post('/webhook/n8n/status', async (req, res) => {
    try {
        ...
        const workflowKey = Object.keys(workflowStatus).find(key => workflowStatus[key].id === workflowId ||
        ...
        try {
            ...
            if (dbStatus && dbStatus.length > 0) {
                ...
                dbStatus.forEach(item => {
                    const key = Object.keys(workflowStatus).find(k => workflowStatus[k].id === item.workflow_id);
                    if (key) {
                        workflowStatus[key] = {
                            ...workflowStatus[key],
                            status: item.status,
