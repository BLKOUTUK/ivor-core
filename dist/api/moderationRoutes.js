...
router.post('/moderate', async (req, res) => {
    ...
    try {
        const { content, moderation_type } = req.body;
 ...
 * Build moderation prompt based on content type
 */
function buildModerationPrompt(content, type) {
    const baseContext = `
 ...
 */
function logModerationDecision(content, result) {
...
router.post('/moderate/test', async (req, res) => {
    ...
    try {
        const results = await Promise.all(testEvents.map(async (content) => {
