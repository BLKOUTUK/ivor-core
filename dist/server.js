...
app.get('/api/crisis/resources', async (req, res) => {
    try {
        ...
        if (categories) {
            ...
            resources = resources.filter(r => r.specializations.some(spec => categoryArray.some(cat => typeof cat === 'string' && spec.toLowerCase().includes(cat.toLowerCase()))));
        }
...
function analyzeSentiment(content) {
    ...
    if (positiveCount > challengingCount && positiveCount > 0)
        return 'positive';
    if (challengingCount > positiveCount && challengingCount > 0)
        return 'challenging';
...
function calculateRiskLevel(urgency, symptoms) {
    if (urgency === 'emergency')
        return 'high';
    if (urgency === 'high')
        return 'moderate';
...
function getRecommendedActions(urgency) {
    switch (urgency) {
        case 'emergency':
