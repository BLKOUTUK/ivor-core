...
exports.DataSovereigntyService = void 0;
...
class DataSovereigntyService {
     ...
     */
    async storeCommunityData(data, sovereigntyRules) {
        try {
            ...
            if (!governanceDecision.approved) {
                throw new CommunityGovernanceService_js_1.CommunityGovernanceRejectionError(`Data storage rejected by community governance: ${governanceDecision.reasons.join(', ')}`);
            }
    ...
    async storeCreatorContent(content) {
        try {
            ...
            if (content.revenueSharing.creatorPercentage < 75) {
                throw new CommunityGovernanceService_js_1.LiberationValidationError(`Creator revenue share must be at least 75%, got ${content.revenueSharing.creatorPercentage}%`);
            }
            ...
            if (!governanceDecision.approved) {
                throw new CommunityGovernanceService_js_1.LiberationValidationError(`Content storage rejected: ${governanceDecision.liberationPrinciples.issues.join(', ')}`);
            }
     ...
     */
    async retrieveCommunityData(dataId, requestingUser) {
        try {
            ...
            if (!governanceDecision.approved) {
                throw new CommunityGovernanceService_js_1.CommunityGovernanceRejectionError(`Data access rejected by community governance: ${governanceDecision.reasons.join(', ')}`);
            }
            // Create audit trail
     ...
     */
    async createCommunityBackup(backupConfig) {
        try {
            ...
            if (!governance.approved) {
                throw new CommunityGovernanceService_js_1.BackupRejectedError(`Community governance rejected backup: ${governance.reasons.join(', ')}`);
            }
     ...
     */
    async executeBackupWithSovereigntyProtection(backupConfig, governance) {
     ...
     */
    async collectLiberationMetrics(communityId) {
        try {
            ...
            if (!governanceDecision.approved) {
                throw new CommunityGovernanceService_js_1.CommunityGovernanceRejectionError(`Analytics collection rejected: ${governanceDecision.reasons.join(', ')}`);
            }
            ...
            const analyticsId = await this.storeLiberationMetrics(communityId, liberationMetrics, governanceDecision.decisionId);
            // Create audit trail
            const auditTrailId = await this.createAuditTrail({
     ...
     */
    async storeWithSovereigntyProtection(data, governanceContext) {
     ...
     */
    async storeWithLiberationProtection(content, governance) {
        const dataId = crypto.randomUUID();
        const timestamp = new Date().toISOString();
        try {
     ...
     */
    async calculateLiberationMetrics(communityId) {
        try {
     ...
     */
    async storeLiberationMetrics(communityId, metrics, governanceDecisionId) {
    ...
    calculateSumMetric(metrics, metricName) {
        if (!metrics || metrics.length === 0)
            return null;
        const values = metrics
            .filter(m => m.metric_name === metricName)
    ...
    calculateLiberationImpact(value, category) {
...
exports.DataSovereigntyService = DataSovereigntyService;
exports.default = DataSovereigntyService;
//# sourceMappingURL=DataSovereigntyService.js.map