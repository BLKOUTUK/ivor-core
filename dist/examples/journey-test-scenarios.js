...
async function runTestScenarios() {
    ...
    const hivCrisisResponse = await journeyService.generateJourneyAwareResponse("I just got diagnosed with HIV and I'm terrified, I don't know what to do", { location: 'london', userId: 'test-user-1' });
    ...
    const prepResponse = await journeyService.generateJourneyAwareResponse("I want to learn about PrEP - how do I get it on the NHS and what does it involve?", { location: 'manchester', userId: 'test-user-2' });
    ...
    const mentalHealthResponse = await journeyService.generateJourneyAwareResponse("I'm struggling with depression and anxiety, looking for therapy through NHS", { location: 'birmingham', userId: 'test-user-3' });
    ...
    const communityResponse = await journeyService.generateJourneyAwareResponse("I'm interested in connecting with other Black queer people for support and healing", { location: 'london', userId: 'test-user-4' });
    ...
    const housingCrisisResponse = await journeyService.generateJourneyAwareResponse("I'm being evicted and need urgent housing help, I'm homeless", { location: 'glasgow', userId: 'test-user-5' });
    ...
    const advocacyResponse = await journeyService.generateJourneyAwareResponse("I want to get involved in organizing and fighting discrimination in my workplace", { location: 'bristol', userId: 'test-user-6' });
    ...
    const ruralResponse = await journeyService.generateJourneyAwareResponse("I live in rural Wales and feel isolated, need support but everything seems to be in cities", { location: 'rural', userId: 'test-user-7' });
...
//# sourceMappingURL=journey-test-scenarios.js.map