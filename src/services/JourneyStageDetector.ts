import { JourneyStage, EmotionalState, UrgencyLevel, CommunityConnectionLevel, UKLocation, JourneyContext, JourneyIndicators } from '../types/journey.js'

/**
 * Journey Stage Detection Engine
 * Recognizes where users are in their UK Black queer liberation journey
 */
export class JourneyStageDetector {
  private journeyIndicators: JourneyIndicators
  ...
  detectJourneyStage(
    ...
    const topStage = Object.entries(stageScores)
      .reduce((a, b) => stageScores[a[0]] > stageScores[b[0]] ? a : b)[0] as JourneyStage
   ...
   */
  private calculateStageScore(input: string, words: string[], indicators: any): number {
    let score = 0
    let totalPossible = 0

    // Check keywords
    const keywordMatches = indicators.keywords?.filter((keyword: string) => 
      input.includes(keyword.toLowerCase())).length || 0
    score += keywordMatches * 2
    totalPossible += indicators.keywords?.length * 2 || 0

    // Check emotional markers
    const emotionalMatches = indicators.emotionalMarkers?.filter((marker: string) => 
      input.includes(marker.toLowerCase())).length || 0
    ...
    if (indicators.urgencyWords) {
      const urgencyMatches = indicators.urgencyWords.filter((word: string) => 
        input.includes(word.toLowerCase())).length
   ...
   */
  private detectUrgencyLevel(input: string, stage: JourneyStage): UrgencyLevel {
    const lowerInput = input.toLowerCase()
   ...
   * Detect UK location
   */
  private detectLocation(locationHint?: string): UKLocation {
    if (!locationHint) return 'unknown'
    ...
    if (location.includes('leeds')) return 'leeds'
    if (location.includes('glasgow')) return 'glasgow'
    if (location.includes('cardiff')) return 'cardiff'
    if (location.includes('belfast')) return 'belfast'
    if (location.includes('bristol')) return 'bristol'
    if (location.includes('liverpool')) return 'liverpool'
    if (location.includes('sheffield')) return 'sheffield'
    if (location.includes('nottingham')) return 'nottingham'
    if (location.includes('brighton')) return 'brighton'
   ...
   */
  private detectCommunityConnection(input: string, userProfile: any): CommunityConnectionLevel {
    const lowerInput = input.toLowerCase()
   ...
   */
  private detectResourcePreference(input: string, userProfile: any): 'phone' | 'online' | 'in_person' | 'flexible' {
    const lowerInput = input.toLowerCase()
