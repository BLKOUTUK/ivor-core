 ...
 */
export class HonestResponseGuard {
  private static readonly MIN_CONFIDENCE_THRESHOLD = 0.3
  private static readonly MIN_RESOURCES_FOR_CONFIDENCE = 1
  private static readonly MIN_KNOWLEDGE_FOR_CONFIDENCE = 1

  /**
   * Evaluate if IVOR has sufficient confidence to provide a response
   */
  static evaluateResponseConfidence(
    query: string,
   ...
   */
  static generateHonestLimitationResponse(
    query: string,
    attemptedResources: UKResource[],
    attemptedKnowledge: KnowledgeEntry[]
  ): JourneyResponse {
...
If this was urgent and I missed that, please call the emergency numbers above immediately.`,
   ...
   */
  private static containsEmergencyIndicators(query: string): boolean {
   ...
   */
  private static identifyQueryTopic(query: string): string {
    const lowerQuery = query.toLowerCase()
   ...
   */
  private static getSuggestedSourcesForTopic(topic: string): Array<{name: string, description: string}> {
    const sourceMap: { [key: string]: Array<{name: string, description: string}> } = {
      'sexual health': [
   ...
   */
  private static getEmergencyResources(): UKResource[] {
    return [
      {
        id: 'emergency-999',
